...
contract BeNFTEARNPASS is Ownable,ReentrancyGuard {
    ...
    mapping(address => WalletList) public whiteList;
    ...
    function addUserWhiteList(address _wallet, uint256 _nftPass) public onlyOwner {
        whiteList[_wallet] = WalletList(true,_nftPass);
    }

     function ChangePercent(uint256 _amount) public onlyOwner {
        PercentEarn = _amount;
    }
    ...
    constructor(address _walletPool, address _TokenRecibe, bool _Iswhitelist) {
        walletPool = _walletPool; // 0x462b9d308933183c9441867e2800b8b79167d33f
        TokenRecibe=_TokenRecibe; // 0x55d398326f99059fF775485246999027B3197955
        _whiteList=_Iswhitelist;
    }
    ...
    function getMaxPerUser(address _wallet) public view returns(uint256) {
        return MaxAmount.mul(whiteList[_wallet].nftPass);
    }
    ...
    function Deposit(uint256 _amount) public nonReentrant ContractLock {
        ...
        if(_whiteList){
             require(whiteList[msg.sender].whitelist,"You are not in whitelist");
         }
        ...
        require(UserInfo.Amount < MaxAmount.mul(whiteList[msg.sender].nftPass),"Limit of user");
    ...
    function Claim() public nonReentrant ContractLock{
        Users memory UserInfo = balanceUser[msg.sender];
        ...
        UserInfo.Earn=0;
        balanceUser[msg.sender]= UserInfo;
        IERC20(TokenRecibe).transfer(msg.sender,UserInfo.Earn);
    }
